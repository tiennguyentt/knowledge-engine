...
DEBATE_SCRIPT = [
    ...
    ("RouterDecision", RT("", [], "", True,
        "R2 gains a policy-amendment launch precondition and an explicit manual-review fallback while it is unmet. The policy change itself is escalated to the human decision list.")),
    ...
    ("Turn", TURN(
        ...
        ["F3", "R2"])),
    ("RouterDecision", RT("", [], "", True,
        "R2's payout language is replaced by the explicit 48-hour bound measured approved_at→paid_at, which doubles as R7's acceptance test. No open quality findings remain.")),
    ...
    ("Turn", TURN(
        ...
        ["F1", "R3"])),
    ("RouterDecision", RT("", [], "", True,
        "The invented 90-day re-verification obligation is removed; R3 is rewritten to once-at-onboarding with risk-based re-verification, tagged for compliance confirmation. P0 grounding failure resolved.")),
    # ---- arbiter ----
    ("AmendmentSet", {
        "amendments": [
    ...
    ("GradeReport", {
        "checklist": [
            ...
            CHECK("D2", "At least 3 edge cases beyond sources", "PASS", "Fallback path, in-flight claims, disbursement failure/idempotency."),
